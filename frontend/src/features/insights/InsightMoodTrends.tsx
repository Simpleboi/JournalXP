...
const POSITIVE_MOODS = ["happy", "excited", "motivated", "grateful", "confident", "hopeful", "relaxed", "calm"];
...
export const InsightMoodTrends: React.FC = () => {
  ...
  const [longestPositiveStreak, setLongestPositiveStreak] = useState({
    current: 0,
    longest: 0,
  });
  ...
  const calculateLongestStreak = (entries: MoodEntry[]): { current: number; longest: number } => {
    ...
    for (const entry of sorted) {
      if (POSITIVE_MOODS.includes(entry.mood.toLowerCase())) {
        if (lastDate === null || differenceInDays(entry.date, lastDate) === 1) {
          currentStreak++;
        } else if (differenceInDays(entry.date, lastDate) > 1) {
          currentStreak = 1;
        }
    ...
    const daysSinceLastEntry = lastEntry ? differenceInDays(today, lastEntry.date) : 999;
              ...
              <div className="space-y-1">
                ...
                <p className="text-xs text-gray-500">
                  {weeklyMoodSummary.topMood && (
                    <>Mostly <span className="font-semibold capitalize">{weeklyMoodSummary.topMood}</span></>
                  )}
                </p>
              </div>
            </CardContent>
            ...
            <CardContent>
              <div className="space-y-1">
                <p className="text-2xl font-bold text-orange-600">
                  {longestPositiveStreak.current} {longestPositiveStreak.current === 1 ? 'day' : 'days'}
                </p>
                <p className="text-xs text-gray-500">
                  Positive moods
                </p>
            ...
            <CardContent>
              <div className="space-y-1">
                <p className="text-2xl font-bold text-yellow-600">
                  {longestPositiveStreak.longest} {longestPositiveStreak.longest === 1 ? 'day' : 'days'}
                </p>
                <p className="text-xs text-gray-500">
                  Your record!
                </p>
            ...
            <CardContent>
              {moodTrendData.filter(d => d.moodScore > 0).length === 0 ? (
                <div className="h-[300px] flex items-center justify-center text-gray-500">
                  <p>No mood data available. Start journaling to track your moods!</p>
                </div>
              ) : (
                <div className="h-[300px] w-full">
                  <ResponsiveContainer width="100%" height="100%">
                    <LineChart data={moodTrendData} margin={{ top: 5, right: 10, left: 0, bottom: 5 }}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  <h4 className="font-semibold text-gray-700">📝 Journaling Impact</h4>
                  <span className={`text-2xl font-bold ${
                    moodCorrelation.withJournaling > 0 ? 'text-green-600' :
                    moodCorrelation.withJournaling < 0 ? 'text-red-600' :
                    'text-gray-600'
                  }`}>
                    {moodCorrelation.withJournaling > 0 ? '+' : ''}{moodCorrelation.withJournaling}%
                  </span>
                </div>
                <div className="h-2 bg-gray-100 rounded-full overflow-hidden">
                  <div
                    className={`h-full ${
                      moodCorrelation.withJournaling > 0 ? 'bg-green-500' :
                      moodCorrelation.withJournaling < 0 ? 'bg-red-500' :
                      'bg-gray-400'
                    }`}
                    style={{
                      width: `${Math.min(Math.abs(moodCorrelation.withJournaling), 100)}%`,
                    }}
                  ></div>
                </div>
                <p className="text-sm text-gray-600">
                  {moodCorrelation.withJournaling > 0 ? (
                    <>Your mood is <strong>{moodCorrelation.withJournaling}% better</strong> on days you journal</>
                  ) : moodCorrelation.withJournaling < 0 ? (
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  <h4 className="font-semibold text-gray-700">✅ Task Completion Impact</h4>
                  <span className={`text-2xl font-bold ${
                    moodCorrelation.withTasks > 0 ? 'text-green-600' :
                    moodCorrelation.withTasks < 0 ? 'text-red-600' :
                    'text-gray-600'
                  }`}>
                    {moodCorrelation.withTasks > 0 ? '+' : ''}{moodCorrelation.withTasks}%
                  </span>
                </div>
                <div className="h-2 bg-gray-100 rounded-full overflow-hidden">
                  <div
                    className={`h-full ${
                      moodCorrelation.withTasks > 0 ? 'bg-green-500' :
                      moodCorrelation.withTasks < 0 ? 'bg-red-500' :
                      'bg-gray-400'
                    }`}
                    style={{
                      width: `${Math.min(Math.abs(moodCorrelation.withTasks), 100)}%`,
                    }}
                  ></div>
                </div>
                <p className="text-sm text-gray-600">
                  {moodCorrelation.withTasks > 0 ? (
                    <>Completing tasks boosts your mood by <strong>{moodCorrelation.withTasks}%</strong></>
                  ) : moodCorrelation.withTasks < 0 ? (
                ...
                {moodDistribution.map((mood, index) => (
                  ...
                  >
                    ...
                    <div className="flex items-center gap-4">
                      <span className="text-sm text-gray-600">{mood.count} times</span>
